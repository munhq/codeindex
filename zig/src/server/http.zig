...
pub const Server = struct {
    ...
    fn write_tools_list(self: *Server, file: conn_mod.Conn, id: ?std.json.Value) !void {
        ...
        // Every tool carries the three MCP review hints explicitly. Directories
        // that review servers (the ChatGPT app submission among them) refuse a
        // tool whose hints are absent, even though the protocol has defaults.
        // All but index_workspace only read: the index, or files under the
        // workspace. index_workspace rebuilds the index, which is derived data,
        // so it is not read-only and not destructive. Nothing here reaches
        // outside the machine, so openWorldHint is false throughout.
        const tool_defs = [_][]const u8{
            // status — no params
            "{\"name\":\"status\",\"description\":\"Get index status: file count, symbol count, indexing state\",\"inputSchema\":{\"type\":\"object\",\"properties\":{}},\"annotations\":{\"readOnlyHint\":true,\"openWorldHint\":false,\"destructiveHint\":false}}",
            // search
            "{\"name\":\"search\",\"description\":\"Search file contents using trigram-accelerated full-text search\",\"inputSchema\":{\"type\":\"object\",\"properties\":{\"query\":{\"type\":\"string\",\"description\":\"Search query string\"},\"limit\":{\"type\":\"integer\",\"description\":\"Max results (default 50)\"}},\"required\":[\"query\"]},\"annotations\":{\"readOnlyHint\":true,\"openWorldHint\":false,\"destructiveHint\":false}}",
            // find_symbol
            "{\"name\":\"find_symbol\",\"description\":\"Find symbol definitions (functions, structs, classes, etc.) by name across all indexed files\",\"inputSchema\":{\"type\":\"object\",\"properties\":{\"name\":{\"type\":\"string\",\"description\":\"Symbol name to search for\"},\"limit\":{\"type\":\"integer\",\"description\":\"Max results (default 50)\"}},\"required\":[\"name\"]},\"annotations\":{\"readOnlyHint\":true,\"openWorldHint\":false,\"destructiveHint\":false}}",
            // find_word
            "{\"name\":\"find_word\",\"description\":\"Look up an exact word/identifier in the inverted word index\",\"inputSchema\":{\"type\":\"object\",\"properties\":{\"word\":{\"type\":\"string\",\"description\":\"Exact word/identifier to find\"},\"limit\":{\"type\":\"integer\",\"description\":\"Max results (default 50)\"}},\"required\":[\"word\"]},\"annotations\":{\"readOnlyHint\":true,\"openWorldHint\":false,\"destructiveHint\":false}}",
            // get_outline
            "{\"name\":\"get_outline\",\"description\":\"Get the structural outline (symbols, line counts) of a specific file\",\"inputSchema\":{\"type\":\"object\",\"properties\":{\"path\":{\"type\":\"string\",\"description\":\"File path relative to workspace root\"}},\"required\":[\"path\"]},\"annotations\":{\"readOnlyHint\":true,\"openWorldHint\":false,\"destructiveHint\":false}}",
            // get_tree — no params
            "{\"name\":\"get_tree\",\"description\":\"Get the directory tree of the indexed workspace with file metadata\",\"inputSchema\":{\"type\":\"object\",\"properties\":{}},\"annotations\":{\"readOnlyHint\":true,\"openWorldHint\":false,\"destructiveHint\":false}}",
            // get_imports
            "{\"name\":\"get_imports\",\"description\":\"Get which files the given file imports/depends on\",\"inputSchema\":{\"type\":\"object\",\"properties\":{\"path\":{\"type\":\"string\",\"description\":\"File path relative to workspace root\"}},\"required\":[\"path\"]},\"annotations\":{\"readOnlyHint\":true,\"openWorldHint\":false,\"destructiveHint\":false}}",
            // get_imported_by
            ...
            // get_hot_files
            "{\"name\":\"get_hot_files\",\"description\":\"Get recently changed files sorted by recency\",\"inputSchema\":{\"type\":\"object\",\"properties\":{\"limit\":{\"type\":\"integer\",\"description\":\"Max results (default 20)\"}}},\"annotations\":{\"readOnlyHint\":true,\"openWorldHint\":false,\"destructiveHint\":false}}",
            // index_workspace
